...
#define MUT_PROB 0.06
#define MAX_GENE_RANGE 255 // gene values can go from 0 to 255
#define MAX_LOOP 10 // max number of loops to generate expression
#define GEN_LIM 300
#define TRAINING_RATIO 0.8 // percentage of the labeled data used as the training set
#define PRESET_RATIO 0.60 // percentage of the initial population set as one of te presets
#define INIT_MUT 10 // number of mutations used on the presets before using them as population
...
#define PRESET3 {204,5,140,231,128,136,148,175,252,223,216,146,231,201,47,89,3,253,141,103,77,184,94,184,139,88,22,39,237,104,122,87,147,148,145,46,239,219,7,209,87,48,129,128,163,109,219,212,198,140}
//20_0.1 0.119586 0.114894 +(-(-(+(/(-(*(3.3)(i))(*(829)(i)))(*(*(6)(k))(*(8.9)(x))))(*(01868.5)(h)))(*(~1)(k)))(p(*(7.4)(j))))(^(*(*(78)(e))(9.7))(^(/(p(*(~018)(x)))(+(/(-(*(3.3)(i))(*(829)(i)))(*(*(6)(k))(*(8.9)(x))))(*(01868.5)(h))))(*(~1)(k))))

// this function shuffles the first num_random positions of a vector whose iterators are begin and end, that is, the first num_random positions are guaranteed to be random, the others might be changed or not
...
double PURGED_PARENTS_RATIO  = 0.1; // percentage of parents that die during select parents phase
int CARDS_CONST = 10; // constant used on the performance function
...
// performance function for the training set
...
// performance function for the validation set
...
void select_parents(){
    ...
    for (int i = 0; i < POP_LIM; i++)
        select_lim[i] /= (total_performance+0.00000001); // trying to avoid division by zero
...
void select(){
    ...
    output << grammar(population[people_performance[0].second]) << ": " <<  CARDS_CONST*1.0/log(1+performance_function(people_performance[0].second)) << " " <<
           CARDS_CONST*1.0/log(1+performance_function2(people_performance[0].second)) << endl; // CARDS_CONST*1.0/log(1+performance_function(people_performance[0].second)) is the cost
...
vector<char> non_t = {'E', 'O', 'V', 'C', 'N', 'n', 'd'}; // non terminal symbols

// applies one step of the grammar
bool mapp(string &exp, int value){ // naming to avoid conflicts with C++ std map
    int i = 0;
    for(; i < exp.size(); i++){
    ...
    switch(actual_exp){
        ...
        case('d'):
            ...
            break;
        default: cout << "exploded" << endl; // this should never be executed
    }
    return true;
}

// map a chromosome to the string it represents
string grammar(vector<int> ind){
    string exp = "E";
    int i = 0;
    int j = 0;
...
//    last_saved = std::chrono::high_resolution_clock::now();
    ...
    return 0.0; // this should never be executed
}
...
int main(){
    ...
    population.resize(MAX_POP_CAP); // pre allocate vector size for speed
    parents.resize(PARENTS_NUM); // pre allocate vector size for speed

    for (int i = 0; i < POP_LIM; i++)
        population[i].resize(IND_PARAMS); // pre allocate vector size for speed

    for (double purge_ratio = 0.1; purge_ratio < 0.41; purge_ratio += 0.1) { // using different purge ratios
        CARDS_CONST = 10;
        PURGED_PARENTS_RATIO = purge_ratio;
        for (int const_it = 0; const_it < 3; const_it++) { // using different cards const
            output = ofstream("results"+to_string(CARDS_CONST)+"_"+to_string(purge_ratio)+".txt", ofstream::out);

            uniform_int_distribution<> i_dis(0, MAX_GENE_RANGE);
            // setting some part of the population as random individuals and the rest as presets
