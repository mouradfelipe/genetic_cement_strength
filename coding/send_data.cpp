...
//    last_saved = std::chrono::high_resolution_clock::now();
    ...
    if (is_number){
        ...
        long double value = stod(expression);
        return value;
    }
    if (expression == "x"){
        return values[0];
    ...
    if (expression[0] == '+' || expression[0] == '-' || expression[0] == '*' || expression[0] == '/' || expression[0] == '^'){
        ...
        long double expression1Value, expression2Value;
...
void readCSV(const string& filename){
    ...
    while(getline(data,line))
    {
        ...
        {
            if(i == 0){
                ids.push_back(stoi(cell.substr(1, cell.size()-2), nullptr, 10));
                i++;
            }
            else
...
int main(){
    ...
    string expr =  "-(*(*(8.0)(e))(*(856)(h)))(/(1999.9)(p(*(*(29.9)(k))(*(2.6)(x)))))";
    //long double perf =  performance_function(exp);
    //cout << exp << ": " << perf << ": " <<  CARDS_CONST*1.0/log(1+perf) << endl;

    ofstream output;
    output = ofstream("sample_results.csv", ofstream::out);
    output << "ID,strength" << endl;
    for(int i = 0; i < parsedCsv.size(); i++){
        output << ids[i] << ", " << sigmoid( 0.001*evaluate_expression(expr, parsedCsv[i])) << endl;
        cout << ids[i] << ", " << sigmoid( 0.001*evaluate_expression(expr, parsedCsv[i])) << endl;
    }
